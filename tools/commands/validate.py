...
def validate_release(release_path: str, generate_md: bool = False):
    ...
    for t in tracks:
        ...
        if wav_file:
            ...
            if not WAV_NAMING_RE.match(wav_file):
                warn(
                    f"Track {num} — '{wav_file}' doesn't match convention "
    ...
    if not errors:
        print(
            f"  {Fore.GREEN}{Style.BRIGHT}ALL CHECKS PASSED — {data.get('catalog', '')} ready.{Style.RESET_ALL}\n"
        )
        if generate_md:
            _generate_release_md(data, path)
    else:
        print(
            f"  {Fore.RED}{Style.BRIGHT}{len(errors)} ERROR(S) — release not ready.{Style.RESET_ALL}"
        )
        for e in errors:
...
| Platform | URL |
|---|---|
| Bandcamp | {data["platforms"].get("bandcamp") or "—"} |
| Spotify | {data["platforms"].get("spotify") or "—"} |
| SoundCloud | {data["platforms"].get("soundcloud") or "—"} |
