...
- **Mechanical texture** — Halftone, scan artifacts, slight degradation are considered additions, not errors.
- **Typography as structure** — Fonts are load-bearing. No decorative type.
...
- Starts with brief: track mood, artist intent, any existing visual references from artist
- Delivers: concept description, compositional structure, color approach, type treatment
...
#### 3. Instagram Visual System
- Defines grid logic (every 3rd post, alternating rhythm, etc.)
- Specifies: post types (announcement, atmospheric, quote, artifact), dimensions, text rules
- Ensures: the feed reads as a coherent document, not a stream of individual decisions

#### 4. Promo Assets (Flyer / Announcements)
- Release announcements, event flyers, repost assets
- Always: date/label/artist hierarchy, no visual noise that competes with information
- Aesthetic rule: a Phantom Grid flyer should be recognizable at thumbnail size
...
1. **Read the brief** — What is the release, the artist, the intended feeling?
2. **Check the system** — Does this require a new element, or can it be solved within existing rules?
3. **Reference check** — Which of the label's reference points (UR, Drexciya, Clone) is most relevant here? Why?
4. **Produce the concept** — Written visual concept first, before any execution detail
5. **Execute or hand off** — Either produces a detailed prompt/spec for image generation, or delivers directly as SVG/CSS/layout instruction
...
- Logo usage rules
- What is forbidden (visual elements that are explicitly off-brand)

Every new asset is checked against this document. Every new system decision is written back into it.
